...
var contextMenuId = chrome.contextMenus.create({
    ...
    onclick: function(info, tab) {
        var account = info.selectionText,
            url = tab.url,
            devRegexp = /^https?:\/\/dev\.qiye/,
            oaRegexp = /^https?:\/\/oa\.qiye/,
            olRegexp = /^https?:\/\/qiye/,
            env = '',
            destUrlHash = {
                'dev': 'https://dev.qiye.qq.com/login',
                'oa': 'https://oa.qiye.qq.com/login',
                'ol': 'https://qiye.qq.com/login'
            },
            destUrl = '',
...
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
        ...
        if(request.from && request.from === 'popup') {
            ...
            console.log('pts pwd' + JSON.stringify(request));
